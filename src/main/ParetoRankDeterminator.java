...
public class ParetoRankDeterminator {
	...
	public void determineParetoRanks1(List<Individual> population) {
	...
	//Alternative version with default sort
	public List<Individual> determineParetoRanks(List<Individual> population) {
		List<Individual> result = new ArrayList<Individual>(population.size());
		int currentRank = 1;
		int n = population.size();
		Individual indiv;
		while (n != 0) {
			for (int i = 0; i < n;i++){
				indiv = population.get(i);
				if (isNotDominated(indiv, population)){
					indiv.setParetoRank(currentRank);
				}
			}
			for (int i = 0; i < n;i++){
